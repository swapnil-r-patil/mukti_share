    ...
    <div className="container mx-auto max-w-7xl py-6 md:py-10 pb-24 px-4 relative overflow-hidden">
       ...
       <div className="absolute top-[5%] right-[-15%] h-[400px] w-[400px] rounded-full bg-orange-500/5 blur-[120px] pointer-events-none" />
        ...
        <div className="mb-8 sm:mb-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
          <div>
            <h2 className="text-2xl sm:text-3xl font-black italic tracking-tighter text-white flex items-center gap-3 sm:gap-4 uppercase">
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-12 relative z-10">
        ...
        <div className="flex flex-col gap-8 md:col-span-12 lg:col-span-4">
          ...
          {isEditing ? (
            <div className="flex flex-col items-center rounded-[2.5rem] bg-slate-950 p-10 border border-white/10 shadow-2xl animate-in zoom-in-95 duration-300">
              <label htmlFor="edit-photo" className="group relative mb-8 flex h-32 w-32 cursor-pointer items-center justify-center overflow-hidden rounded-[2rem] border-2 border-dashed border-white/10 bg-white/5 transition-all hover:border-orange-500">
                {editPhoto ? (
                  <img src={editPhoto} alt="Profile" className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center bg-orange-500 text-5xl font-black text-white italic">
                    {(editName || user.name).charAt(0)}
                  </div>
                )}
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 text-white opacity-0 transition-opacity group-hover:opacity-100">
              ...
              <div className="w-full space-y-4">
                <input value={editName} onChange={e => setEditName(e.target.value)} placeholder="Full Identity" className="w-full rounded-2xl border border-white/5 bg-black px-6 py-4 text-sm text-white font-bold outline-none focus:border-orange-500 transition-all" />
                <input value={editSkill} onChange={e => setEditSkill(e.target.value)} placeholder="Professional Skill" className="w-full rounded-2xl border border-white/5 bg-black px-6 py-4 text-sm text-white font-bold outline-none focus:border-orange-500 transition-all" />
                <input value={editLocation} onChange={e => setEditLocation(e.target.value)} placeholder="Current Base" className="w-full rounded-2xl border border-white/5 bg-black px-6 py-4 text-sm text-white font-bold outline-none focus:border-orange-500 transition-all" />
              </div>
              
              <div className="mt-8 flex w-full gap-4">
                <button onClick={() => setIsEditing(false)} className="flex-1 py-4 rounded-2xl bg-white/5 border border-white/10 text-white text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">Cancel</button>
            ...
            <div className="flex flex-col items-center rounded-[3rem] bg-slate-950 p-12 text-center border border-white/5 shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative mb-6">
                 <div className="h-32 w-32 rounded-[2.5rem] bg-orange-500 flex items-center justify-center text-5xl font-black text-white italic shadow-[0_0_40px_rgba(249,115,22,0.3)] overflow-hidden border-4 border-white/10">
                    {user.photo ? <img src={user.photo} alt="Profile" className="h-full w-full object-cover" /> : user.name.charAt(0)}
                 </div>
                 <div className="absolute -bottom-2 -right-2 p-3 rounded-2xl bg-slate-900 border border-orange-500/30 text-orange-500 shadow-xl">
                    <Award size={20} />
                 </div>
              </div>
              <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase">{user.name}</h2>
              ...
              <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
                ...
                <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 rounded-full bg-white/5 border border-white/10 px-4 py-2 text-[9px] font-black text-slate-400 hover:text-white hover:bg-white/10 transition-all uppercase tracking-widest">
                  <Edit2 size={12} /> Edit
                </button>
              </div>
            </div>
          ...
          {/* Details List */}
          <div className="rounded-[2.5rem] bg-slate-950 border border-white/5 shadow-2xl overflow-hidden">
            <div className="px-8 py-5 border-b border-white/5 bg-white/[0.02] font-black text-[10px] uppercase tracking-[0.3em] text-slate-600">
               Core Registry Metadata
            </div>
            <div className="p-4 space-y-2">
              {infoItems.map((item, i) => (
                <div key={item.label} className="flex items-center gap-4 px-5 py-4 rounded-2xl hover:bg-white/5 transition-all group border border-transparent hover:border-white/5">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-500/10 text-orange-500 border border-orange-500/10 group-hover:scale-110 transition-transform">
                    <item.icon size={18} />
                  </div>
                  <div>
                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest leading-none mb-1">{item.label}</div>
        ...
        <div className="flex flex-col gap-8 md:col-span-12 lg:col-span-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {user.role === "worker" ? (
              <>
                <div className="rounded-[2.5rem] bg-slate-950 p-8 border border-white/5 text-center shadow-xl group hover:border-orange-500/20 transition-all">
                  <div className="mb-4 flex justify-center text-orange-500">
                    <Star size={32} className="fill-orange-500 drop-shadow-[0_0_10px_rgba(249,115,22,0.5)]" />
                  </div>
                  <div className="text-4xl font-black text-white italic tracking-tighter">{avgRating}</div>
                  <div className="mt-2 text-[9px] font-black uppercase tracking-[0.4em] text-slate-600">Average Rating</div>
                </div>
                ...
                <div className="rounded-[2.5rem] bg-slate-950 p-8 border border-white/5 text-center shadow-xl group hover:border-orange-500/20 transition-all">
                  ...
                  <div className="text-4xl font-black text-white italic tracking-tighter">{verifications.length}</div>
              ...
              <>
                <div className="rounded-[2.5rem] bg-slate-950 p-8 border border-white/5 text-center shadow-xl group hover:border-orange-500/20 transition-all">
                  ...
                  <div className="text-4xl font-black text-white italic tracking-tighter">{user.trustScore || 0}</div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <button onClick={() => navigate("/report")} className="group relative rounded-[2.5rem] bg-slate-950 p-8 border border-white/5 text-left shadow-xl hover:border-orange-500/30 transition-all active:scale-[0.98]">
                <div className="flex items-center gap-6">
                   ...
                   <div className="flex-1">
                      <span className="block text-lg font-black text-white italic uppercase tracking-tighter mb-1">Work Report</span>
              ...
              <button onClick={handleLogout} className="group relative rounded-[2.5rem] bg-slate-950 p-8 border border-white/5 text-left shadow-xl hover:border-red-500/30 transition-all active:scale-[0.98]">
                <div className="flex items-center gap-6">
          ...
          <div className="rounded-[2.5rem] bg-slate-900/50 border border-white/5 p-8 text-center border-dashed">
